...
def convert_move_for_server(move, color):
    ...
    col = color
    message = {"from": move[0], "to": move[1], "turn": col}
...
def position(row, col):
    ...
    return str(alphabets[col]) + str(row + 1)
...
def main(player_name, color, timeo= 60, host= 'localhost'):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        ...
        while True:
            ...
            if len(len_bytes) == 4:
                ...
                move = my_ai_logic(board_state, color)
